...
const Page = ({ params }: { params: Promise<{ userId: string }> }) => {
  ...
  return (
    <Card className="w-screen h-screen bg-black border-none rounded-none relative">
      ...
      {postCount == 0 ? (
        <div className="text-white flex gap-3 border border-gray-600 border-r-0 border-l-0 border-t-0 p-4 items-center">
          <svg
            aria-label="Camera"
        ...
        <CardFooter className="p-0 flex flex-wrap gap-[1.5px]">
          {user?.posts.map((post) => {
            return (
              <Link
                key={post._id}
                className="p-0 w-[33%] mt-1"
                href={`/posts/onePost/${post._id}`}
              >
                <Image
                  src={post.postImg}
